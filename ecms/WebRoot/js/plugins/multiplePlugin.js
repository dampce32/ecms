...
(function($) {  
  ...
  $.fn.multipleInit = function() {
	  ...
	  $(optionsDG).datagrid({
		...
		columns:[[
		      {field:'ck',checkbox:true},
		      {field:'option',title:'选项',width:50,align:"center"},
		      {field:'options',title:'内容',width:250,align:"center",
					editor:{
						type:'text'	
					}}
		]],
	...
	$(editDialog).dialog({  
	  ...
	  var onDeleteOption = function(){
		  ...
		  for(var i=0;i<=rows.length;i++){
			  rows[i].option=arr[i];
			  $(optionsDG).datagrid('refreshRow',i);
		  }
	...
	var setValue = function(){
		...
		var checkedRows=$(optionsDG).datagrid('getChecked');
		if(checkedRows.length==0){
			$.messager.alert('提示','请勾选答案','warning');
			return false;
		}
		if(checkedRows.length==1){
			$.messager.alert('提示','请至少勾选两个答案','warning');
			return false;
		}
		var answerList = new Array();
		for(var i=0;i<checkedRows.length;i++){
			answerList.push(checkedRows[i].option);
		}
		var answerStr = answerList.join('');
		$('#answer',editForm).val(answerStr);
	...
	var onOpen = function(){
		...
		for(var i=0;i<selectRow.optionCount;i++){
			switch (i) {
				case 0:
					$(optionsDG).datagrid('appendRow',{
						option:arr[i],
						options:selectRow.option0
					 });
					break;
				case 1:
					$(optionsDG).datagrid('appendRow',{
						option:arr[i],
						options:selectRow.option1
					 });
					break;
				case 2:
					$(optionsDG).datagrid('appendRow',{
						option:arr[i],
						options:selectRow.option2
					 });
					break;
				case 3:
					$(optionsDG).datagrid('appendRow',{
						option:arr[i],
						options:selectRow.option3
					 });
					break;
				case 4:
					$(optionsDG).datagrid('appendRow',{
						option:arr[i],
						options:selectRow.option4
					 });
					break;
				case 5:
					$(optionsDG).datagrid('appendRow',{
						option:arr[i],
						options:selectRow.option5
					 });
					break;
				case 6:
					$(optionsDG).datagrid('appendRow',{
						option:arr[i],
						options:selectRow.option6
					 });
					break;
				case 7:
					$(optionsDG).datagrid('appendRow',{
						option:arr[i],
						options:selectRow.option7
					 });
					break;
				case 8:
					$(optionsDG).datagrid('appendRow',{
						option:arr[i],
						options:selectRow.option8
					 });
					break;
				case 9:
					$(optionsDG).datagrid('appendRow',{
						option:arr[i],
						options:selectRow.option9
					 });
					break;
				default:
					break;
		...
		var rows=$(optionsDG).datagrid('getRows');
		var answerList = selectRow.answer.split('');
		for(var i=0;i<rows.length;i++){
			for(var j=0;j<answerList.length;j++){
				if(answerList[j]==rows[i].option){
					$(optionsDG).datagrid('checkRow',i);
				}
			}
		}
	};
