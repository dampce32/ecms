  ...
  $.fn.nationInit = function() {
	  ...
	  $(viewList).datagrid({
		...
		columns:[[
			...
			{field:'nationName',title:'民族',width:150,align:"center"},
		]],
	...
	var search = function(flag){
		var nationName=$('#nameSearch',queryContent).val();
		var status = $('#statusSearch',queryContent).combobox('getValue');
		//取得列表信息
		var url = 'dict/queryNation.do';
		var options = $(viewList).datagrid('options');
		var content = {nationName:nationName,
	...
	var getTotal = function(content){
		var url = "dict/getTotalCountNation.do";
	...
	var setValue = function(){
		var nationName = $.trim($('#nationName',editForm).val());
		if(''==nationName){
			$.messager.alert('提示','请填写民族','warning');
			return false;
		}
		//状态
	...
	var onSave = function(){
		$(editForm).form('submit',{
			...
			success: function(data){
				...
				if(result.isSuccess){
					var fn = function(){
						var nationId = $('#nationId',editForm).val();
						//新增
						if(nationId==''){
							search(true);
						}else{
	...
	var onMulUpdateState = function(status){
		...
		$.messager.confirm("提示","确定要"+msg+"记录?",function(t){ 
			if(t){
				var url = 'dict/mulUpdateStateNation.do';
	...
	var onMove = function(direction){
		...
		var nationId = selectRow.nationId;
		var nationName = selectRow.nationName;
		var status = selectRow.status;
		//后台更新排序
		var url = "dict/updateArrayNation.do";
		var content = {nationId:nationId,updateNationId:updateRow.nationId};
		var result = syncCallService(url,content);
		if(result.isSuccess){
