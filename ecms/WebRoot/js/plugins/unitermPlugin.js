...
(function($) {  
  ...
  $.fn.unitermInit = function() {
	...
	var setValue = function(){
		...
		$('#answer',editForm).val(checkedRows[0].option);
	...
	var onOpen = function(){
		...
		var rows=$(optionsDG).datagrid('getRows');
		for(var i=0;i<rows.length;i++){
			if(selectRow.answer==rows[i].option){
	...
	var search = function(flag){
		...
		var content = {'group.groupId':groupId,'publishTeacher.teacherId':teacherId,
				subjectType:'单项选择',status:status,difficulty:difficulty};
