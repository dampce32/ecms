...
(function($) {  
  ...
  $.fn.areaInit = function() {
	  ...
	  $('#citySearch',queryContent).combobox({
		  width:150,
	  ...
	  $('#provinceSearch',queryContent).combobox({
		  ...
		  onSelect: function(record){  
			  var url = 'dict/queryComboboxCity.do?province.provinceId='+record.provinceId;
			  $('#citySearch',queryContent).combobox({
				  url:url
			  });
	      }
	  ...
	  $(viewList).datagrid({
		...
		columns:[[
			...
			{field:'areaCode',title:'区县编号',width:100,align:"center"},
			{field:'areaName',title:'区县名称',width:200,align:"center"}
		]],
	...
	var onAdd = function(){
		$(editForm).form('clear');
		init();
		$(editDialog).dialog({title: '添加参赛区县'});	
		$(editDialog).dialog('open');
	}
	//保存前的赋值操作
	var setValue = function(){
		var areaCode = $.trim($('#areaCode',editForm).val());
		if(''==areaCode){
			$.messager.alert('提示','请填写区县编号','warning');
			return false;
		}
		var areaName = $.trim($('#areaName',editForm).val());
		if(''==areaName){
			$.messager.alert('提示','请填写区县名称','warning');
			return false;
		}
		return true;
	}
	...
	var onUpdate = function(){
		if(!$('#update_'+id,$this).is(":hidden")){
			...
			$(editForm).form('clear');
			init();
			$('#province',editForm).combobox('select',selectRow.provinceId);
			$('#city',editForm).combobox('select',selectRow.cityId);
			$(editForm).form('load',selectRow);
			$(editDialog).dialog({title: '修改参赛区县'});	
			$(editDialog).dialog('open');
		}
	 }
	//查询
	    var search = function(){
		var areaCode = $('#areaCodeSearch',queryContent).val();
		var areaName = $('#areaNameSearch',queryContent).val();
		var provinceId =  $('#provinceSearch',queryContent).combobox('getValue');
		var cityId =  $('#citySearch',queryContent).combobox('getValue');
		
		var content = {areaCode:areaCode,areaName:areaName,'province.provinceId':provinceId,'city.cityId':cityId};
		
		$(viewList).datagrid({
			url:"dict/queryArea.do",
			queryParams:content
		});
		checkBtnRight();
	}
	...
	var onMulDelete = function(){
		...
		$.messager.confirm("提示！","确定要删除选中的记录?",function(t){ 
			...
			for(var i=0;i<rows.length;i++){
				idArray.push(rows[i].areaId);
			}
			var ids = idArray.join(CSIT.join);
			var url = "dict/mulDeleteArea.do";
			var content = {ids:ids};
			$.post(url,content,
				function(result){
					if(result.isSuccess){
	...
	var onMove = function(direction){
		...
		//后台更新排序
		var url = "dict/updateArrayArea.do";
		var content = {areaId:areaId,updateAreaId:updateRow.areaId};
