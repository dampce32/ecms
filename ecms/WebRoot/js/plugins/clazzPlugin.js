...
(function($) {  
  ...
  $.fn.clazzInit = function() {
	...
	var search = function(flag){
		var clazzName=$('#nameSearch',queryContent).val();
		var status = $('#statusSearch',queryContent).combobox('getValue');
		//取得列表信息
		var url = 'dict/queryClazz.do';
		var options = $(viewList).datagrid('options');
		var content = {clazzName:clazzName,
	...
	var getTotal = function(content){
		var url = "dict/getTotalCountClazz.do";
	...
	var setValue = function(){
		var clazzName = $.trim($('#clazzName',editForm).val());
		if(''==clazzName){
			$.messager.alert('提示','请填写班级','warning');
			return false;
		}
		//状态
	...
	var onSave = function(){
		$(editForm).form('submit',{
			...
			success: function(data){
				...
				if(result.isSuccess){
					var fn = function(){
						var clazzId = $('#clazzId',editForm).val();
						//新增
						if(clazzId==''){
							search(true);
						}else{
	...
	var onMulDelete = function(){
		...
		$.messager.confirm("提示！","确定要删除选中的记录?",function(t){ 
			...
			var url = "dict/mulDeleteClazz.do";
			var content = {ids:ids};
			$.post(url,content,
				function(result){
					if(result.isSuccess){
	...
	var onMulUpdateState = function(status){
		...
		$.messager.confirm("提示","确定要"+msg+"记录?",function(t){ 
			if(t){
				var url = 'dict/mulUpdateStateClazz.do';
	...
	var onMove = function(direction){
		...
		var clazzName = selectRow.clazzName;
		var status = selectRow.status;
		//后台更新排序
		var url = "dict/updateArrayClazz.do";
		var content = {clazzId:clazzId,updateClazzId:updateRow.clazzId};
		var result = syncCallService(url,content);
		if(result.isSuccess){
